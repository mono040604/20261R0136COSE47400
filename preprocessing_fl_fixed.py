"""
LoRA + FocalLoss 修复版：neg_reg 堵住负样本梯度消失

核心修复：FL = FocalLoss + neg_reg * BCE_on_negatives

FocalLoss 在 multi-label 下负样本梯度被 (1-p_t)^γ 压到接近 0，
160 个"说不"信号消失 → 模型过预测 → macro F1 低。
neg_reg 给每个负样本一笔固定 BCE 保底工资，不管 p_t 多高都不消失。
"""
import torch
import clip
import os
...
print(f"run start — LoRA + FocalLoss FIXED (neg_reg)")
...
SAVE_MODLE_PATH = "./trained_clip_lora_fl_fixed"
...
LORA_ALPHA = 64
SEED = 42
GAMMA = 2.0         # FocalLoss gamma, keep original
NEG_REG = 1.0        # ★ 核心修复：负样本 BCE 保底权重（0.2 不够，负样本仍在漂）
torch.manual_seed(SEED)
torch.cuda.manual_seed(SEED)
device = "cuda"

print(f"Hyperparams: gamma={GAMMA}, neg_reg={NEG_REG}")
...
print("CLIP ViT-B/16 + LoRA + FocalLoss(fixed) configured")
...
class FocalLossFixed(torch.nn.Module):
    """
    FocalLoss + negative regularization.

    L = FL(alpha, gamma) + neg_reg * BCE_on_negatives

    FocalLoss 压没了负样本梯度 → 模型过预测。
    neg_reg 给每个负样本一笔固定 BCE 保底工资，确保"说不"永不被静音。

    对比原始 FL:
      FL:          负样本 logit=-4, 梯度≈0.000006 (被压 3100 倍)
      FL+neg_reg:  负样本 logit=-4, 梯度≈0.000006 + 0.2×0.018 ≈ 0.0036
                    保底梯度是原来的 600 倍
    """
    ...
    def forward(self, logits, targets):
        ...
        # Alpha: only weight positive samples
        if self.alpha is not None:
            alpha_t = targets * self.alpha + (1 - targets) * 1.0
            loss = alpha_t * loss

        # ★ neg_reg: 负样本保底 BCE，堵住梯度消失
        if self.neg_reg > 0:
            neg_mask = (1 - targets)               # 只对负样本生效
...
loss_fn = FocalLossFixed(alpha=class_weight, gamma=GAMMA, neg_reg=NEG_REG, reduction='none')
print(f"Using FocalLossFixed (gamma={GAMMA}, neg_reg={NEG_REG})")
...
for epoch in range(EPOCHS):
    ...
    if avg_loss < best_loss:
        best_loss = avg_loss
        best_model_path = os.path.join(SAVE_MODLE_PATH, "model_best_clip+lora_fl_fixed")
        model.save_pretrained(best_model_path)
        torch.save(model.state_dict(), os.path.join(SAVE_MODLE_PATH, "model_best_clip+lora_fl_fixed.pt"))
